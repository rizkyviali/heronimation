...
import { heroVariants, getVariant } from "@/lib/registry";
...
export default async function VariantPage({ params }: { params: Promise<{ variant: string }> }) {
  ...
  const filePath = path.join(process.cwd(), "src/components/heroes", variant.componentFile);
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <main className="mx-auto max-w-7xl px-6 py-12">
        ...
        <section className="mb-10">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-widest text-zinc-500">Preview</h2>
          <HeroPreview variant={variant} />
        </section>

        <section className="mb-10">
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-widest text-zinc-500">Source Code</h2>
          <p className="mb-4 text-sm text-zinc-500">
            Copy the file below into your project. Requires{" "}
            <code className="rounded bg-zinc-800 px-1.5 py-0.5 text-xs text-lime-400">framer-motion</code>,{" "}
            <code className="rounded bg-zinc-800 px-1.5 py-0.5 text-xs text-lime-400">tailwindcss</code>, and{" "}
            <code className="rounded bg-zinc-800 px-1.5 py-0.5 text-xs text-lime-400">next/font/google</code>.
          </p>
          <div className="mb-4">
            <FontSetupBlock />
          </div>
        ...
        <section>
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-widest text-zinc-500">Props</h2>
